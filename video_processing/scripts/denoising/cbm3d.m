function [ yRGB_est, runtime, PSNR, SNR ] = cbm3d( zRGB, yRGB, sigma, profile, print_to_screen, colorspace )
%
%
%  [ yRGB_est, runtime, PSNR, SNR ] = cbm3d( zRGB, yRGB, sigma, profile, print_to_screen, colorspace )
%
%
%  TO DO
%
%    - s'assurer que s pas bonne image orig on renvoie PSNR = [0 0], et SNR
%    uassi
%
%
...
if strcmp(profile, 'vn_old') && (sigma > 40)
...
if strcmp(transform_3rd_dim_name, 'haar') || strcmp(transform_3rd_dim_name(end-2:end), '1.1')
...
if (yRGB_is_invalid_image)
    print_to_screen = 0;
    fprintf('yRGB does not seem to be the original of zRGB.\nSNR cannot be computed.\n');
end
...
if (~yRGB_is_invalid_image) % then we assume yRGB is a valid image
    PSNR(1, 1) = 10*log10(1/mean((yRGB(:)-zRGB(:)).^2));
    PSNR(1, 2) = 10*log10(1/mean((yRGB(:)-yRGB_est(:)).^2));
    SNR (1, 1) = 10*log10( mean(yRGB(:).^2) / (mean((yRGB(:)-zRGB(:)).^2)) );
    SNR (1, 2) = 10*log10( mean(yRGB(:).^2) / (mean((yRGB(:)-yRGB_est(:)).^2)) );
end


runtime = wiener_elapsed_time + estimate_elapsed_time;
